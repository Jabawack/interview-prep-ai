...
from ..agents.graph import AGENT_LABELS, AGENT_LOOKUP
from ..config import settings
from ..services import intent_parser, response_composer
    ...
    async def _worker(task_id: str, agent_key: str, message: str, thread_id: str) -> None:
        ...
        try:
            async for event in _stream_agent_events(
                agent, message, thread_id, recursion_limit, task_id=task_id,
            ):
...
async def process_message(message: str) -> AsyncGenerator[dict, None]:
    ...
    if len(specialist_tasks) == 1:
        ...
        async for event in _stream_agent_events(
            AGENT_LOOKUP[agent_key], msg, thread_id, recursion_limit, task_id=task_id,
        ):
            if event["event"] == _SENTINEL_EVENT:
                agent_results[task_id] = event["data"]["final_text"]
            else:
        ...
        async for event in _run_parallel_specialists(
            specialist_tasks, recursion_limit, agent_results,
        ):
            yield event
    ...
    if agent_results:
        outputs = [
            response_composer.parse_agent_output(text)
            for text in agent_results.values()
        ]
        output = response_composer.merge_outputs(outputs)
    else:
        output = response_composer.AgentOutput(
            error="No agent produced results."
        )
