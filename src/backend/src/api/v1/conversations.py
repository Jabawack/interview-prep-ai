...
async def list_messages(conversation_id: UUID, limit: int = 100, offset: int = 0):
    ...
    return [
        MessageOut(
            ...
            role=r["role"],
            content=r["content"],
            reasoning_trace=r["reasoning_trace"] if isinstance(r["reasoning_trace"], dict) else None,
            created_at=r["created_at"],
        )
        for r in rows
    ]
