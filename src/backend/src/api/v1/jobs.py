...
from ...models.schemas import JobPostingOut, SaveJobRequest
...
async def get_job(job_id: UUID):
    ...
    row = await pool.fetchrow(
        "SELECT id, title, company, location, url, salary_range, parsed_data, created_at FROM job_postings WHERE id = $1",
        job_id,
    )
    if not row:
...
async def create_job(body: SaveJobRequest):
    """Manually save a job posting."""
    pool = await get_pool()
    row = await pool.fetchrow(
        """
        INSERT INTO job_postings (title, company, location, url, salary_range, raw_content, parsed_data)
        VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb)
        RETURNING id, title, company, location, url, salary_range, parsed_data, created_at
        """,
        body.title,
        body.company,
        body.location,
