    ...
    return json.dumps(
        {
            ...
            "source": "none",
            "message": (
                f"No stored data for {company_name}. Consider researching on Glassdoor or LeetCode."
            ),
        }
    )
        ...
        return json.dumps(
            {
                "company": row["name"],
                "interview_rounds": (
                    json.loads(row["interview_rounds"]) if row["interview_rounds"] else []
                ),
                "glassdoor_rating": (
                    float(row["glassdoor_rating"]) if row["glassdoor_rating"] else None
                ),
        ...
        return json.dumps(
            {
                ...
                "common_questions": (
                    json.loads(row["common_questions"]) if row["common_questions"] else []
                ),
                "glassdoor_rating": float(row["glassdoor_rating"])
                if row["glassdoor_rating"]
                else None,
                "culture_notes": row["culture_notes"],
                "last_updated": row["updated_at"].isoformat() if row["updated_at"] else None,
            }
    ...
    return json.dumps(
        {
            ...
            "culture_notes": None,
            "message": (
                f"No profile found for {company_name}. I'll share general knowledge instead."
            ),
        }
    )
