...
from src.models.schemas import CompanyProfileOut

router = APIRouter(prefix="/companies", tags=["companies"])


@router.get("/", response_model=list[CompanyProfileOut])
async def list_companies(limit: int = 50, offset: int = 0) -> list[CompanyProfileOut]:
...
async def get_company(company_id: UUID) -> CompanyProfileOut:
...
async def get_company_by_name(name: str) -> CompanyProfileOut:
...
def _row_to_profile(row: asyncpg.Record) -> CompanyProfileOut:
    return CompanyProfileOut(
        id=row["id"],
        name=row["name"],
        industry=row["industry"],
        interview_rounds=(
            json.loads(row["interview_rounds"])
            if isinstance(row["interview_rounds"], str)
            else row["interview_rounds"]
        ),
        common_questions=(
            json.loads(row["common_questions"])
            if isinstance(row["common_questions"], str)
            else row["common_questions"]
        ),
